...
struct CollisionDetectorBoxToBox {
    ...
    static void detect(T1& obj1, size_t idx1, T2& obj2, size_t idx2) {
        ...
        if ( idx1 >= obj1_hitboxes.size() ) {
            return;
        }

        const auto& obj2_hitboxes = obj2.hitboxes();
        if ( idx2 >= obj2_hitboxes.size() ) {
            return;
        }

        const auto& h1 = obj1_hitboxes[idx1];
        const auto& h2 = obj2_hitboxes[idx2];
        ...
        if ((l1 <= r2) && (l2 <= r1) && (t1 <= b2) && (t2 <= b1)) {
            const mgc::collision::BoxCollisionInfo info1 = {h1, h2, idx1, idx2};
            obj1.on_hit_box_to_box(obj2, info1);

            const mgc::collision::BoxCollisionInfo info2 = {h2, h1, idx2, idx1};
    ...
    static void detect(T1& obj1, T2& obj2) {
        ...
        for ( size_t h1_index = 0; h1_index < obj1_hitboxes.size(); h1_index++ ) {
            for ( size_t h2_index = 0; h2_index < obj2_hitboxes.size(); h2_index++ ) {
                detect(obj1, h1_index, obj2, h2_index);
            }
        }
    }

    template <typename T1, typename T2>
    static void detect(T1& obj1, size_t idx1, T2& obj2) {

        const auto& obj2_hitboxes = obj2.hitboxes();

        for ( size_t h2_index = 0; h2_index < obj2_hitboxes.size(); h2_index++ ) {
            detect(obj1, idx1, obj2, h2_index);
        }
    }

    template <typename T1, typename T2>
    static void detect(T1& obj1, T2& obj2, size_t idx2) {

        const auto& obj1_hitboxes = obj1.hitboxes();

        for ( size_t h1_index = 0; h1_index < obj1_hitboxes.size(); h1_index++ ) {
            detect(obj1, h1_index, obj2, idx2);
        }
    }
};
...
struct CollisionDetectorBoxToMap {
    ...
    void detect(ObjT& obj, size_t idx, MapT& map, CornerPushDirection push_dir = CornerPushDirection::None) {
        ...
        if ( idx >= obj_hitboxes.size() ) {
            return;
        }

        const auto& h = obj_hitboxes[idx];
        if ( !h.enabled ) {
            return;
        }
        ...
        for ( uint16_t j = qt_ ; j <= qb_; j++ ) {
            for ( uint16_t i = ql_; i <= qr_; i++ ) {
                ...
                if ( (map_cell_value  & 0x80) != 0 ) {
                    ...
                    const mgc::collision::MapCollisionInfo info = {h, idx, map_cell_value, j, i};
        ...
        if ( hit_count_ > 0 ) {
            ...
            const mgc::collision::MapPushbackInfo info = { h, idx, pushback };
    ...
    void detect(ObjT& obj, MapT& map, CornerPushDirection push_dir = CornerPushDirection::None) {
        ...
        for ( size_t h_index = 0; h_index < obj_hitboxes.size(); h_index++ ) {
            detect(obj, h_index, map, push_dir);
        }
    }
