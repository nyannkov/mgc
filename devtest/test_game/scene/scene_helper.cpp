...
void update(
    ...
    if ( !talkflow.in_progress() ) {
        ...
        if ( scx.stage ) {
            scx.stage->detect_hit(scx.player);
            if ( scx.objs ) {
                for ( auto* enemy : scx.objs->enemies() ) {
                    scx.stage->detect_hit(*enemy);
                }
            }
        }
        ...
        if ( scx.objs ) {
            for ( auto* enemy : scx.objs->enemies() ) {
                ...
                scx.stage->detect_hit(*enemy);
                ColBox2BoxT::detect(*enemy, scx.player);
                ColBox2BoxT::detect(*enemy, scx.player.attack());
            ...
            for ( auto* prop : scx.objs->props() ) {
                ColBox2BoxT::detect(*prop, scx.player);
            }
            for ( auto* civil : scx.objs->civils() ) {
                ColBox2BoxT::detect(*civil, scx.player);
            }
            for ( auto* item : scx.objs->items() ) {
                ColBox2BoxT::detect(*item, scx.player);
                if ( item->collected() ) {
                    item->despawn();
                }
            }
        ...
        if ( scx.evts ) {
            for ( auto* cs : scx.evts->events() ) {
                ColBox2BoxT::detect(*cs, scx.player);
            }
        }
...
bool check_scene_transition_request(
    ...
    if ( game_over_effect.has_finished() ) {
        id_next = SceneId::Id_999_GameOver;
        return true;
    }
