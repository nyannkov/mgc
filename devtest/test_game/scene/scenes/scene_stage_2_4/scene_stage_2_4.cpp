...
Scene_Stage2_4::Scene_Stage2_4(GameContext& ctx) 
            : SceneBase(ctx),
              stage_(scx_.timer),
              scene_objects_(scx_),
              event_objects_(scx_, scene_objects_) {

    set_id(SceneId::Stage2_4);
...
void Scene_Stage2_4::init() {
    ...
    stage_.setup(stage::StageId::Stage2_4);
    stage_.set_position({0, 0});

    camera_.set_target(scx_.player);
    camera_.set_x_follow_setting(MGC_CELL2PIXEL(6), MGC_CELL2PIXEL(12), MGC_CELL2PIXEL(1));
...
void Scene_Stage2_4::draw(mgc::graphics::Framebuffer& fb) {
