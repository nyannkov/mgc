...
Scene_Stage3_4::Scene_Stage3_4(GameContext& ctx) 
            : SceneBase(ctx),
              stage_(scx_.timer),
              scene_objects_(scx_) {
              

    set_id(SceneId::Stage3_4);
...
void Scene_Stage3_4::init() {
    
    if ( id_prev() == SceneId::Stage3_2 ) {
        scx_.player.reset_state_for_placement(
            {MGC_CELL2PIXEL(56), MGC_CELL2PIXEL(59)},
            PlayerAnimState::StandRight
        );
    } else if ( id_prev() == SceneId::Stage3_5 ) {
        scx_.player.reset_state_for_placement(
            {MGC_CELL2PIXEL(38), MGC_CELL2PIXEL(30)},
            PlayerAnimState::StandRight
        );
    } else {
        scx_.player.reset_state_for_placement(
    ...
    stage_.setup(stage::StageId::Stage3_4);
...
void Scene_Stage3_4::draw(mgc::graphics::Framebuffer& fb) {
