...
Scene_Stage3_5::Scene_Stage3_5(GameContext& ctx) 
            : SceneBase(ctx),
              stage_(scx_.timer),
              scene_objects_(scx_),
              event_objects_(scx_, scene_objects_) {

    set_id(SceneId::Stage3_5);
...
void Scene_Stage3_5::init() {
    
    if ( id_prev() == SceneId::Stage3_4 ) {
        ...
        scx_.player.reset_state_for_placement(
            {MGC_CELL2PIXEL(11), MGC_CELL2PIXEL(35)},
    ...
    stage_.setup(stage::StageId::Stage3_5);
...
void Scene_Stage3_5::draw(mgc::graphics::Framebuffer& fb) {
