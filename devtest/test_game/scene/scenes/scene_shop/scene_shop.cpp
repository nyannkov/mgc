...
Scene_Shop::Scene_Shop(GameContext& ctx) 
            : SceneBase(ctx),
              stage_(scx_.timer),
              scene_objects_(scx_),
...
void Scene_Shop::init() {
    ...
    camera_.set_target_position_in_camera_space(
        {TARGET_X_IN_CAMERA-36, TARGET_Y_IN_CAMERA}
    );
    camera_.set_x_follow_setting(MGC_CELL2PIXEL(1), MGC_CELL2PIXEL(7));
...
void Scene_Shop::draw(mgc::graphics::Framebuffer& fb) {
