...
Scene_Stage1_4::Scene_Stage1_4(GameContext& ctx) 
            : SceneBase(ctx),
              stage_(scx_.timer),
              scene_objects_(scx_),
              event_objects_(scx_, scene_objects_, camera_) {

    set_id(SceneId::Stage1_4);
...
void Scene_Stage1_4::init() {
    
    if ( id_prev() == SceneId::Shop ) {
        scx_.player.reset_state_for_placement(
            {MGC_CELL2PIXEL(21), MGC_CELL2PIXEL(8)},
            PlayerAnimState::StandRight
        );
    } else {
        scx_.player.reset_state_for_placement(
            {MGC_CELL2PIXEL(28), MGC_CELL2PIXEL(30)},
    ...
    stage_.setup(stage::StageId::Stage1_4);
    ...
    camera_.set_y_follow_setting(MGC_CELL2PIXEL(6), MGC_CELL2PIXEL(26));
...
void Scene_Stage1_4::draw(mgc::graphics::Framebuffer& fb) {
