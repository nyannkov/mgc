...
Scene_Stage1_5::Scene_Stage1_5(GameContext& ctx) 
            : SceneBase(ctx),
              stage_(scx_.timer),
              scene_objects_(scx_) {

    set_id(SceneId::Stage1_5);
...
void Scene_Stage1_5::init() {
    ...
    } else if ( id_prev() == SceneId::Stage1_6 ) {
        scx_.player.reset_state_for_placement(
    ...
    stage_.setup(stage::StageId::Stage1_5);
    ...
    camera_.set_y_follow_setting(MGC_CELL2PIXEL(6), MGC_CELL2PIXEL(58));
...
void Scene_Stage1_5::draw(mgc::graphics::Framebuffer& fb) {
