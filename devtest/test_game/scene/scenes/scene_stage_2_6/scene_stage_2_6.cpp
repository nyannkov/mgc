...
Scene_Stage2_6::Scene_Stage2_6(GameContext& ctx) 
            : SceneBase(ctx),
              stage_(scx_.timer),
              scene_objects_(scx_, stage_),
              event_objects_(scx_, scene_objects_) {

    set_id(SceneId::Stage2_6);
...
void Scene_Stage2_6::init() {
    ...
    } else if ( id_prev() == SceneId::Stage2_7 ) {
        scx_.player.reset_state_for_placement(
            {MGC_CELL2PIXEL(156), MGC_CELL2PIXEL(11)},
    ...
    stage_.setup(stage::StageId::Stage2_6);
    stage_.set_position({0, 0});

    camera_.set_target(scx_.player);
    camera_.set_x_follow_setting(MGC_CELL2PIXEL(1), MGC_CELL2PIXEL(153), MGC_CELL2PIXEL(7));
    camera_.set_y_follow_setting(MGC_CELL2PIXEL(1), MGC_CELL2PIXEL(7), MGC_CELL2PIXEL(7));
...
void Scene_Stage2_6::draw(mgc::graphics::Framebuffer& fb) {
